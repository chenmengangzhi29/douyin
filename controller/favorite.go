...
func FavoriteAction(c *gin.Context) {
	...
	if favo.Id==0{
		if action=="1"{
			...
			last:=models.Favorite{}
			models.DB.Table("favorite").Last(&last)
			models.DB.Table("favorite").Select("id","user_id","video_id").Create(&models.Favorite{Id: last.Id+1,UserId:user_id,VideoId: video_id})
		}
	}else{
		if action=="2"{
			models.DB.Table("video").Where("id=?",video_id).Find(&video)
